 ...
 */
class BaseServiciosController extends AppController {
		...
		if (!empty($this -> request -> data)) {
			...
			if($correos_validos) {
				...
				if ($this -> Proyecto -> save($this -> request -> data)) {
					...
					if($this -> request -> data['Proyecto']['servicio_id'] == 2) {
						$this -> redirect(array('action' => 'view', "controller" => "empresas", $this -> request -> data["Proyecto"]["empresa_id"], "proyectos"));
					} elseif($this -> request -> data['Proyecto']['servicio_id'] == 3) {
						$this -> redirect(array('action' => 'view', "controller" => "empresas", $this -> request -> data["Proyecto"]["empresa_id"], "calidad"));
					} elseif($this -> request -> data['Proyecto']['servicio_id'] == 4) {
						$this -> redirect(array('action' => 'view', "controller" => "empresas", $this -> request -> data["Proyecto"]["empresa_id"], "ingenieria"));
					}
				} else {
	...
	public function admin_add2($id = null) {
		...
		if (!empty($this -> request -> data)) {
			...
			if ($this -> Proyecto -> save($this -> request -> data)) {
				...
				//$this->redirect(array('action' => 'view',"controller"=>"empresas",$this->data["Proyecto"]["cliente_id"]));
			} else {
				$empresaId = $this -> request -> data["Proyecto"]["empresa_id"];
	...
	public function AJAX_cambiarEstado() {
		$this -> Proyecto -> recursive = -1;
		if($this -> Proyecto -> read(null, $this -> data['id'])) {
			$this -> Proyecto -> id = $this -> data['id'];
			$this -> Proyecto -> saveField('estado_proyecto_id', $this -> data['estado_proyecto_id']);
	...
	public function AJAX_guardarDesarrollo() {
		...
		if($this -> Proyecto -> read(null, $this -> data['id'])) {
			$this -> Proyecto -> id = $this -> data['id'];
			$this -> Proyecto -> saveField('desarrollo', $this -> data["texto"]);
			echo "Se ha actualizado el campo";
		} else {
			echo "NO";
		}
		Configure::Write("debug", 0);
	...
	public function AJAX_subirControlEjecucion() {
		if($this -> Proyecto -> read(null, $this -> data['id'])) {
			$this -> Proyecto -> id = $this -> data['id'];
			$this -> Proyecto -> saveField('control_ejecucion', $this -> data['path']);
			echo "Se ha subido con exito el control de ejecución";
		} else {
			echo "NO";
		}
		Configure::Write("debug", 0);
